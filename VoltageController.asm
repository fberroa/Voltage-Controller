...
Switch_off			BTFSC	GPIO,	4
					GOTO	Amp_check
					GOTO	OFF
Amp_check			MOVLW	0x83				;Lee AN0 y se guarda en la variable CURRENT.
					MOVWF	ADCON0
goback				BTFSS	PIR1,	6
					GOTO	goback
					BCF		PIR1,	6
					...
					MOVWF	CURRENT
					MOVF	THRESHOLD,	0		
					SUBWF	CURRENT,0			;Realiza la resta de entre CURRENT y THRESHOLD (CURRENT - THRESHOLD)
					BTFSC	STATUS,C			;Verificar si la resta produce Borrow
					GOTO	ON					;Corriente menor al umbral
					GOTO	OFF					;Corriente mayor al umbral	
					
;ON/OFF
ON					BSF		GPIO,	5			;Encender motor del molino
					GOTO	Switch_off
OFF					BCF		GPIO, 	5			;Apagar motor del molino por sobrecarga o por fin de operación
					GOTO	Switch_on
